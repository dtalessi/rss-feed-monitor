#!/usr/bin/env python3
"""
RSS Feed Monitor - Single Run Version for GitHub Actions
Checks the feed once and exits (designed to be run on a schedule)
"""
...
Read more: {entry.get('link', '')}
    ...
    def check_feed_once(self):
        """Check the RSS feed once and process new posts"""
        try:
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Checking feed...")
            ...
            if len(self.seen_posts) == 0:
                print(f"First run - marking {len(feed.entries)} existing posts as seen (no emails sent)")
                for entry in feed.entries:
                    post_id = self._get_post_id(entry)
                    self.seen_posts.add(post_id)
                self._save_seen_posts()
                print("Setup complete! Future posts will trigger email notifications.")
                return
